...
/**
 * A class that handles autonomous driving for swerve drive robots. This includes motion profiling
 * and active PID correction.
 * @param xPID The PID controller for the x axis
 * @param yPID The PID controller for the y axis
 * @param twistPID The PID controller for the twist axis
 * @param twistTrapConstraints The constraints for the twist PID controller
 * @param angleDeadzone The deadzone for the twist PID controller
 * @param positionDeadzone The deadzone for the x and y PID controllers
 * @param swo The swerve odometry object
 * @param swerveSystem The swerve drive train object
 * @param gyro The gyro object
 * @param debugLogging Whether or not to log debug information to the console
 * @param field2d The field2d object to update
 */
class SwerveAuto(
    ...
    var desiredPosition: FieldPosition = FieldPosition(0.0, 0.0, 0.0)
        set(value) {
            ...
            if (!field2d.isEmpty) {
                val changedPos =
                        Field2d.toWPILIBFieldPosition(FieldPosition(value.x, value.y, value.angle))
                field2d.get().getObject("DesiredPosition").pose =
                        Pose2d(
                                Translation2d(changedPos.x, changedPos.y),
                                Rotation2d(changedPos.angleRadians)
                        )
            }
